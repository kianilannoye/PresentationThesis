...
\begin{frame}{Decision space $\rightarrow$ decision tree}
	\begin{block}{Converting decision spaces to decision trees}
		One-to-one mapping from decision tree to space is gone because the order is lost during conversion from DT to DS. Therefore, a \textbf{heuristic} approach must be taken, identifying \textbf{hyperplane candidates} and calculating a metric to choose the `best' plane.
	\end{block}
\end{frame}
...
\begin{frame} {Headache dataset}
	\begin{itemize}
		\item Extract features from the collected headache data
